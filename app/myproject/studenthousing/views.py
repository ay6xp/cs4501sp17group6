    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if User.objects.all().filter(username=username).count() > 0:
                ...
                new_user = models.User(username=username, password=hashers.make_password(request.POST['password']),
                                       email=request.POST['email'], phone_num=request.POST['phone_num'])
                try:
                    new_user.save()
                except db.Error:
                    response_data = {}
...
def user_from_name(request, username):
    if request.method == 'GET':
        ...
        if User.objects.all().filter(username=username).exists():
            ...
            data = {
                'username': curr_user.username, 'password': curr_user.password, 'email': curr_user.email,
                'phone_num': curr_user.phone_num, 'joined_date': curr_user.joined_date, 'pk': curr_user.pk
            }
            response_data = {}
            response_data['ok'] = True
